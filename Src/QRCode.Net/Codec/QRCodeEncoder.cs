using System.Text;
using QRCode.Net.Codec.Util;
using QRCode.Net.Properties;

namespace QRCode.Net.Codec;
...
public class QRCodeEncoder
{
    ...
    /// <returns></returns>
    private static bool[,] Convert2Matrix(bool[][] map)
    {
        var matrix = new bool[map.Length, map[0].Length];
        for (int row = 0; row < map.Length; row++)
        {
            for (int col = 0; col < map[row].Length; col++)
            {
                matrix[row, col] = map[row][col];
            }
        }
        return matrix;
    }

    public virtual Stream Encode(string content, Encoding encoding)
    {
        var array = calQrcode(encoding.GetBytes(content));
        var matrix = Convert2Matrix(array);
        var stream = Png.PngQrCodeGenerator.Instance.GeneratePng(matrix, qrCodeScale);
        return stream;
    }

    public virtual Stream Encode(string content)
    {
